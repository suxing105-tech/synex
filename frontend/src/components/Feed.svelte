            ...
            {#each col.items as it (it.id)}
              ...
              >
                ...
                {#if $comfyuiStatus.running && it.has_workflow && (hoveredId === it.id || $selectedIdStore === it.id || $multiSelectedIds.has(it.id))}
                  <!--
                    ComfyUI 一键打开：直接放真 logo PNG，无外圈装饰。
                    顶角优先级高于 ♥：hover/选中时让出右上角给 logo，
                    避免两者叠加成糊一团。
                  -->
                  <img
                    src="/comfyui-logo.png"
                    alt="在 ComfyUI 中打开工作流"
                    title="在 ComfyUI 中打开工作流"
                    class="comfyui-open-btn absolute top-1 right-1 w-[22px] h-[22px] cursor-pointer hover:brightness-125 transition z-[2] drop-shadow-md"
...
<style>
  ...
  /* ComfyUI 一键打开：img 直接渲染 PNG 真 logo，无外圈；
     hover 仅 brightness 微提亮（drop-shadow-md 在 Tailwind class 里）。 */
  .thumb-img {
