...
with (case / 'stdout.log').open('w', encoding='utf-8') as out, (case / 'stderr.log').open('w', encoding='utf-8') as err:
    ...
    try:
        ...
        print(json.dumps({'passed': True, 'checks': ['startup', 'directory_import', 'source_display_rename', 'persistent_reorder', 'mixed_delete_create', 'directory_delete', 'stats'], 'case': str(case)}, ensure_ascii=False))
    finally:
        proc.terminate()
